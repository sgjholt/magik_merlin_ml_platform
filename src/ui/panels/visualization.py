...
class VisualizationPanel:
    ...
    def __init__(self, experiment_manager=None) -> None:
        ...
        self.category_select = pn.widgets.Select(
            ...
            options=["Data Exploration", "Model Evaluation", "Experiment Tracking"],
            value="Data Exploration",
        )

        # Visualization type selection (updated based on category)
        self.viz_type_select = pn.widgets.Select(
            name="Visualization Type", options=[], disabled=True
        )

        # Refresh button
        self.refresh_button = pn.widgets.Button(
            name="Refresh Visualization", button_type="primary", width=150
        )
    ...
    def _create_panel(self) -> pn.Column:
        ...
        return pn.Column(
            ...
            pn.Row(
                ...
                pn.Column(self.status_indicator, margin=(10, 0, 0, 0)),
            ),
            # Main content row
            pn.Row(
        ...
        if viz_type == "Model Comparison":
            ...
            if results_data is not None:
                ...
                self._show_no_data_message(
                    "No model results available. Run an experiment first."
                )

        elif viz_type == "ROC Curves":
            # This would require actual model predictions
            self._show_placeholder_message(
                "ROC Curves", "Run classification experiments to see ROC curves"
            )
            ...
            self._show_placeholder_message(
                "Confusion Matrix",
                "Run classification experiments to see confusion matrix",
            )

        elif viz_type == "Feature Importance":
            # This would require trained models with feature importance
            self._show_placeholder_message(
                "Feature Importance", "Train models to see feature importance"
            )
    ...
    def _create_experiment_tracking_viz(self, viz_type: str) -> None:
        ...
        if viz_type == "Experiment History":
                        ...
                        exp_data.append(
                            {
                                ...
                                "accuracy": exp.metrics.get("best_accuracy", 0)
                                if exp.metrics
                                else 0,
                            }
                        )
            ...
            try:
                completed_experiments = self.experiment_manager.list_experiments(
                    status="completed"
                )
                if len(completed_experiments) >= 2:
                    ...
                    comparison_df = self.experiment_manager.get_experiment_comparison(
                        exp_ids
                    )
                    ...
                    self._show_no_data_message(
                        "Need at least 2 completed experiments for comparison"
                    )
            except Exception as e:
                ...
                if completed_experiments:
                    ...
                    if latest_exp.metrics:
                        ...
                        results_data = [
                            {
                                "Model": "Best Model",
                                "Accuracy": latest_exp.metrics.get(
                                    "best_accuracy", 0.85
                                ),
                                "Precision": latest_exp.metrics.get(
                                    "best_precision", 0.82
                                ),
    ...
    def _set_status(self, message: str) -> None:
        ...
        self.status_indicator.object = (
            f"<div style='color: #666; font-style: italic;'>{message}</div>"
        )
    ...
    def update_data(self, data: pd.DataFrame) -> None:
        ...
        self.logger.info(
            f"Updated visualization data: {data.shape if data is not None else 'None'}"
        )
